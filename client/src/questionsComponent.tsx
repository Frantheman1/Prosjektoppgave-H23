...
export class QuestionsList extends Component {
  ...
  mounted() {
    ...
    answerService.getAnswerCounts().then((answersCount) => {
  ...
  render() {
    return (
      <>
        ...
        <Button.Success onClick={() => history.push(`/answers/new/` + this.props.match.params.id)}>
          Answer
        </Button.Success>
        <Button.Success
            ...
            if (comment.questionId == this.question.questionId) {
              return (
                <Row>
                  ...
                  <Button.Success
                    onClick={() =>
                      history.push(`/comment/${comment.commentId}/edit/${this.question.questionId}`)
                    }>
                    Edit
                  </Button.Success>
                </Row>
        ...
        <Card title="Answers">
          <Button.Success onClick={() => this.sortByProperty('score')}>
            Sort by Answer Count
          </Button.Success>
          <Button.Success onClick={() => this.sortByProperty('modifiedAt')}>
            Sort by Date
          </Button.Success>
          {this.displayedAnswer.map((answer) => (
            <Row>
              ...
              <Button.Success onClick={() => history.push(`/answers/${answer.answerId}/edit`)}>
                Edit
              </Button.Success>
              <Row>
                <Button.Success
                  onClick={() => this.voteForAnswer(answer.answerId, answer.userId, 1)}>
                  Upvote
                </Button.Success>
                <Button.Success
                  onClick={() => this.voteForAnswer(answer.answerId, answer.userId, 0)}>
                  Downvote
                </Button.Success>
                <Form.Checkbox
                  checked={answer.isAccepted}
                  onChange={(e) =>
                    this.toggleAcceptance(answer.answerId, e.target.checked)
                  }></Form.Checkbox>
                ...
                <Button.Success
                  onClick={() =>
                    history.push(`/comments/answer/${answer.answerId}/new/${answer.questionId}`)
                  }>
                  Comment
                </Button.Success>
              </Row>
                  ...
                  if (comment.answerId == answer.answerId) {
                    return (
                      <Row>
                        ...
                        <Button.Success
                          onClick={() =>
                            history.push(`/comment/${comment.commentId}/edit/${answer.questionId}`)
                          }>
                          Edit
                        </Button.Success>
                      </Row>
      ...
      .then(() => {
        ...
        if (previouslyAcceptedAnswer) {
          answerService
            ...
            .catch((error) =>
              console.error(
                'Error updating previously accepted answer on the server: ' + error.message,
              ),
            );
        }
      })
...
export class QuestionsNew extends Component {
  ...
  tag: string =''
  tags: { tagsId: number}[] = []
  displayedTags: {name: string , id: number}[] = []
  render() {
    return (
      <>
